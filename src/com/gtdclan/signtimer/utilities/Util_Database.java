...
public abstract class Util_Database {
	 ...
	 * @param url
	 *        Location of the database. For example: jdbc:sqlite:{DIR}{NAME}.db
	 * @param username
	 *        Username required to access the database
	 * @param password
	 *        Password belonging to the username, may be empty
	 * @param isolation
	 *        Isolation type. For example: SERIALIZABLE, also see TransactionIsolation
	 * @param logging
	 *        If set to false, all logging will be disabled
	 * @param rebuild
	 *        If set to true, all tables will be dropped and recreated. Be sure to create a backup before doing so!
	 */
		...
		try {
			...
			while ((currentLine = scriptReader.readLine()) != null) {
				...
				if (currentLine.startsWith("create table")) {
					...
					previousLine = previousLine.substring(
					    0,
					...
					if (alterTableLine[3].startsWith("add constraint")) {
						...
						if (addConstraintLine[3].startsWith("foreign key")) {
							...
							// on the end
							String constraintLine = String.format(
							    "%s %s %s",
							    addConstraintLine[1],
							    addConstraintLine[2],
							    addConstraintLine[3]);
							...
							throw new RuntimeException(
							    "Unsupported action encountered: ALTER TABLE using ADD CONSTRAINT with " + addConstraintLine[3]);
						}
					}
				}
			}
